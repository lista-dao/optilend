...
contract WithdrawIntegrationTest is BaseTest {
  ...
  function testWithdrawAssets(uint256 amountSupplied, uint256 amountBorrowed, uint256 amountWithdrawn) public {
    ...
    (uint256 returnAssets, uint256 returnShares) = morpho.withdraw(
      marketParams,
      amountWithdrawn,
      0,
      address(this),
    ...
    assertEq(morpho.position(id, address(this)).supplyShares, expectedSupplyShares, "supply shares");
    assertEq(morpho.market(id).totalSupplyShares, expectedSupplyShares, "total supply shares");
    assertEq(morpho.market(id).totalSupplyAssets, amountSupplied - amountWithdrawn, "total supply");
    ...
    assertEq(loanToken.balanceOf(address(morpho)), amountSupplied - amountBorrowed - amountWithdrawn, "morpho balance");
  }
  ...
  function testWithdrawShares(uint256 amountSupplied, uint256 amountBorrowed, uint256 sharesWithdrawn) public {
    ...
    assertEq(morpho.position(id, address(this)).supplyShares, expectedSupplyShares, "supply shares");
    assertEq(morpho.market(id).totalSupplyAssets, amountSupplied - expectedAmountWithdrawn, "total supply");
    assertEq(morpho.market(id).totalSupplyShares, expectedSupplyShares, "total supply shares");
  ...
  function testWithdrawAssetsOnBehalf(uint256 amountSupplied, uint256 amountBorrowed, uint256 amountWithdrawn) public {
    ...
    assertEq(returnAssets, amountWithdrawn, "returned asset amount");
    assertEq(returnShares, expectedWithdrawnShares, "returned shares amount");
    assertEq(morpho.position(id, ONBEHALF).supplyShares, expectedSupplyShares, "supply shares");
    assertEq(morpho.market(id).totalSupplyAssets, amountSupplied - amountWithdrawn, "total supply");
    assertEq(morpho.market(id).totalSupplyShares, expectedSupplyShares, "total supply shares");
