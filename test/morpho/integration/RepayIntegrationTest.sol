...
contract RepayIntegrationTest is BaseTest {
  ...
  function testRepayAssets(
    ...
    assertEq(morpho.market(id).totalBorrowAssets, amountBorrowed - amountRepaid, "total borrow");
  ...
  function testRepayShares(
    ...
    assertEq(morpho.position(id, ONBEHALF).borrowShares, expectedBorrowShares, "borrow shares");
    assertEq(morpho.market(id).totalBorrowAssets, amountBorrowed - expectedAmountRepaid, "total borrow");
    assertEq(morpho.market(id).totalBorrowShares, expectedBorrowShares, "total borrow shares");
    assertEq(loanToken.balanceOf(RECEIVER), amountBorrowed, "RECEIVER balance");
    assertEq(
