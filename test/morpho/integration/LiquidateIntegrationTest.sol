...
contract LiquidateIntegrationTest is BaseTest {
  ...
  function testLiquidateSeizedInputNoBadDebtRealized(LiquidateTestParams memory params, uint256 amountSeized) public {
    ...
    assertEq(morpho.market(id).totalBorrowAssets, expectedBorrowed, "total borrow");
    assertEq(morpho.market(id).totalBorrowShares, expectedBorrowShares, "total borrow shares");
    assertEq(morpho.position(id, BORROWER).collateral, expectedCollateral, "collateral");
  ...
  function testLiquidateSharesInputNoBadDebtRealized(LiquidateTestParams memory params, uint256 sharesRepaid) public {
    ...
    uint256 maxSharesRepaid = (params.amountCollateral - 1)
      ...
      .toSharesDown(morpho.market(id).totalBorrowAssets, morpho.market(id).totalBorrowShares);
    ...
    uint256 expectedSeized = sharesRepaid
      .toAssetsDown(morpho.market(id).totalBorrowAssets, morpho.market(id).totalBorrowShares)
  ...
  function testLiquidateBadDebtRealized(
    ...
    params.borrowSharesBeforeLiquidation = morpho.position(id, BORROWER).borrowShares;
    params.totalBorrowSharesBeforeLiquidation = morpho.market(id).totalBorrowShares;
    params.totalBorrowBeforeLiquidation = morpho.market(id).totalBorrowAssets;
    params.totalSupplyBeforeLiquidation = morpho.market(id).totalSupplyAssets;
    ...
    (uint256 returnSeized, uint256 returnRepaid) = morpho.liquidate(marketParams, BORROWER, amountCollateral, 0, hex"");
    ...
    assertEq(morpho.position(id, BORROWER).collateral, 0, "collateral");
    ...
    assertEq(morpho.position(id, BORROWER).borrowShares, 0, "borrow shares");
    assertEq(morpho.market(id).totalBorrowShares, 0, "total borrow shares");
    assertEq(
      morpho.market(id).totalBorrowAssets,
