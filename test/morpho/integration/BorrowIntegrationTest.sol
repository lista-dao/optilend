...
contract BorrowIntegrationTest is BaseTest {
  ...
  function testBorrowAssets(
    ...
    assertEq(morpho.market(id).totalBorrowAssets, amountBorrowed, "total borrow");
    assertEq(morpho.position(id, BORROWER).borrowShares, expectedBorrowShares, "borrow shares");
    assertEq(morpho.position(id, BORROWER).borrowShares, expectedBorrowShares, "total borrow shares");
  ...
  function testBorrowShares(
    ...
    (uint256 returnAssets, uint256 returnShares) = morpho.borrow(marketParams, 0, sharesBorrowed, BORROWER, RECEIVER);
    vm.stopPrank();
    ...
    assertEq(morpho.market(id).totalBorrowAssets, expectedAmountBorrowed, "total borrow");
    assertEq(morpho.position(id, BORROWER).borrowShares, sharesBorrowed, "borrow shares");
    assertEq(morpho.position(id, BORROWER).borrowShares, sharesBorrowed, "total borrow shares");
  ...
  function testBorrowAssetsOnBehalf(
    ...
    (uint256 returnAssets, uint256 returnShares) = morpho.borrow(marketParams, amountBorrowed, 0, ONBEHALF, RECEIVER);

    assertEq(returnAssets, amountBorrowed, "returned asset amount");
    assertEq(returnShares, expectedBorrowShares, "returned shares amount");
    assertEq(morpho.position(id, ONBEHALF).borrowShares, expectedBorrowShares, "borrow shares");
    assertEq(morpho.market(id).totalBorrowAssets, amountBorrowed, "total borrow");
    assertEq(morpho.market(id).totalBorrowShares, expectedBorrowShares, "total borrow shares");
    assertEq(loanToken.balanceOf(RECEIVER), amountBorrowed, "borrower balance");
    assertEq(loanToken.balanceOf(address(morpho)), amountSupplied - amountBorrowed, "morpho balance");
  }
  ...
  function testBorrowSharesOnBehalf(
    ...
    (uint256 returnAssets, uint256 returnShares) = morpho.borrow(marketParams, 0, sharesBorrowed, ONBEHALF, RECEIVER);

    assertEq(returnAssets, expectedAmountBorrowed, "returned asset amount");
    assertEq(returnShares, sharesBorrowed, "returned shares amount");
    assertEq(morpho.position(id, ONBEHALF).borrowShares, sharesBorrowed, "borrow shares");
    assertEq(morpho.market(id).totalBorrowAssets, expectedAmountBorrowed, "total borrow");
    assertEq(morpho.market(id).totalBorrowShares, sharesBorrowed, "total borrow shares");
    assertEq(loanToken.balanceOf(RECEIVER), expectedAmountBorrowed, "borrower balance");
    assertEq(loanToken.balanceOf(address(morpho)), amountSupplied - expectedAmountBorrowed, "morpho balance");
  }
}
