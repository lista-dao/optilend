...
contract SupplyIntegrationTest is BaseTest {
  ...
  function testSupplyAssets(uint256 amount) public {
    ...
    assertEq(morpho.market(id).totalSupplyAssets, amount, "total supply");
  ...
  function testSupplyShares(uint256 shares) public {
    ...
    assertEq(morpho.position(id, ONBEHALF).supplyShares, shares, "supply shares");
    assertEq(morpho.market(id).totalSupplyAssets, expectedSuppliedAmount, "total supply");
    assertEq(morpho.market(id).totalSupplyShares, shares, "total supply shares");
    assertEq(loanToken.balanceOf(SUPPLIER), 0, "SUPPLIER balance");
