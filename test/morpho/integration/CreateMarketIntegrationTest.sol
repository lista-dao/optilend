...
contract CreateMarketIntegrationTest is BaseTest {
  ...
  function testCreateMarketWithEnabledIrmAndLltv(MarketParams memory marketParamsFuzz) public {
    ...
    assertEq(morpho.market(marketParamsFuzzId).lastUpdate, block.timestamp, "lastUpdate != block.timestamp");
    assertEq(morpho.market(marketParamsFuzzId).totalSupplyAssets, 0, "totalSupplyAssets != 0");
    assertEq(morpho.market(marketParamsFuzzId).totalSupplyShares, 0, "totalSupplyShares != 0");
    assertEq(morpho.market(marketParamsFuzzId).totalBorrowAssets, 0, "totalBorrowAssets != 0");
    assertEq(morpho.market(marketParamsFuzzId).totalBorrowShares, 0, "totalBorrowShares != 0");
    assertEq(morpho.market(marketParamsFuzzId).fee, 0, "fee != 0");
  }
