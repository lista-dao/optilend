...
import { SharesMathLib } from "../../src/morpho/libraries/SharesMathLib.sol";
...
contract ReallocateWithdrawTest is IntegrationTest {
  ...
  function testReallocateWithdrawMax() public {
    ...
    emit EventsLib.ReallocateWithdraw(
      ...
      allMarkets[0].id(),
      CAP2,
      morpho.position(allMarkets[0].id(), address(vault)).supplyShares
    );
    emit EventsLib.ReallocateWithdraw(
      ALLOCATOR_ADDR,
      allMarkets[1].id(),
      CAP2,
      morpho.position(allMarkets[1].id(), address(vault)).supplyShares
    );
    emit EventsLib.ReallocateWithdraw(
      ALLOCATOR_ADDR,
      allMarkets[2].id(),
      CAP2,
      morpho.position(allMarkets[2].id(), address(vault)).supplyShares
    );

    vm.prank(ALLOCATOR_ADDR);
    vault.reallocate(allocations);

    assertEq(morpho.position(allMarkets[0].id(), address(vault)).supplyShares, 0, "morpho.supplyShares(0)");
    assertEq(morpho.position(allMarkets[1].id(), address(vault)).supplyShares, 0, "morpho.supplyShares(1)");
    assertEq(morpho.position(allMarkets[2].id(), address(vault)).supplyShares, 0, "morpho.supplyShares(2)");
  ...
  function testReallocateWithdrawIncreaseSupply() public {
    ...
    emit EventsLib.ReallocateSupply(
      ...
      allMarkets[2].id(),
      3 * CAP2,
      3 * morpho.position(allMarkets[2].id(), address(vault)).supplyShares
    );
