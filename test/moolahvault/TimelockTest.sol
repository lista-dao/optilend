...
contract TimelockTest is IntegrationTest {
  ...
  function testDeployMoolahVaultAboveMaxTimelock(uint256 timelock) public {
  ...
  function testDeployMoolahVaultBelowMinTimelock(uint256 timelock) public {
  ...
  function testSubmitCapDecreased(uint256 cap) public {
    ...
    vm.expectEmit(address(vault));
    emit EventsLib.SetCap(CURATOR_ADDR, id, cap);
  ...
  function testSubmitCapIncreased(uint256 cap) public {
    ...
    vm.expectEmit(address(vault));
    emit EventsLib.SubmitCap(CURATOR_ADDR, id, cap);
    vm.prank(CURATOR_ADDR);
    vault.submitCap(marketParams, cap);
