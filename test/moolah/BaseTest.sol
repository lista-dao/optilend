...
contract BaseTest is Test {
  ...
  function setUp() public virtual {
    ...
    moolah = newMoolah(OWNER, OWNER, address(oracle));
  ...
  function newMoolah(address admin, address manager, address _oracle) internal returns (IMoolah) {
    Moolah moolahImpl = new Moolah();

    ERC1967Proxy moolahProxy = new ERC1967Proxy(
      address(moolahImpl),
      abi.encodeWithSelector(moolahImpl.initialize.selector, admin, manager, _oracle)
    );
